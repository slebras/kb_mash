...
function kb_mash(url, auth, auth_cb, timeout, async_job_check_time_ms, service_version) {
    var self = this;
    ...
    var _auth_cb = auth_cb;
     ...
     this.run_mash_dist_search = function (params, _callback, _errorCallback) {
        ...
        return json_call_ajax(_url, "kb_mash.run_mash_dist_search",
            [params], 1, _callback, _errorCallback);
    };
 
     this.run_mash_sketch = function (MashSketchParams, _callback, _errorCallback) {
        if (typeof MashSketchParams === 'function')
            throw 'Argument MashSketchParams can not be a function';
        if (_callback && typeof _callback !== 'function')
            throw 'Argument _callback must be a function if defined';
        if (_errorCallback && typeof _errorCallback !== 'function')
            throw 'Argument _errorCallback must be a function if defined';
        if (typeof arguments === 'function' && arguments.length > 1+2)
            throw 'Too many arguments ('+arguments.length+' instead of '+(1+2)+')';
        return json_call_ajax(_url, "kb_mash.run_mash_sketch",
            [MashSketchParams], 1, _callback, _errorCallback);
    };
    ...
    this.status = function (_callback, _errorCallback) {
        ...
        return json_call_ajax(_url, "kb_mash.status",
