...
sub new
{
    ...
    my $self = {
	client => kb_mash::kb_mashClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
=head2 run_mash_dist_search

  $results = $obj->run_mash_dist_search($params)
...
<pre>
$params is a kb_mash.MashParams
$results is a kb_mash.MashResults
MashParams is a reference to a hash where the following keys are defined:
...
# Authentication: required
	...
	Bio::KBase::Exceptions::ArgumentValidationError->throw(error =>
							       "Invalid argument count for function run_mash_dist_search (received $n, expecting 1)");
    }
    {
	my($params) = @args;
	...
	my @_bad_arguments;
        ...
        if (@_bad_arguments) {
	    my $msg = "Invalid arguments passed to run_mash_dist_search:\n" . join("", map { "\t$_\n" } @_bad_arguments);
    ...
    my $result = $self->{client}->call($url, $self->{headers}, {
	    method => "kb_mash.run_mash_dist_search",
	    params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
        ...
        Bio::KBase::Exceptions::HTTP->throw(error => "Error invoking method run_mash_dist_search",
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for kb_mash::kb_mashClient\n";
    }
    if ($sMajor == 0) {
        warn "kb_mash::kb_mashClient version is $svr_version. API subject to change.\n";
    }
}
...
package kb_mash::kb_mashClient::RpcClient;
